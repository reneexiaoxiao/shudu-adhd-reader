...
const READER_MESSAGE_TYPES = new Set([
  "SHUDU_TRANSLATE",
  "SHUDU_TRANSLATION_TEST",
  "SHUDU_TRANSLATION_POPUP_ACTION"
]);

chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  // 合并版同时加载收藏后台。不是译读模块的消息必须交给后续监听器，
  // 不能抢先返回“未知请求”，否则收藏弹窗永远拿不到真实结果。
  if (!READER_MESSAGE_TYPES.has(message?.type)) return false;
  const run = async () => {
